...
def update_ontology(item):
        ...
        if kind =='object':
            ...
            IsBig = input("Is object big? reply in true or false\n")
...
ex:{item} rdf:type ex:object;
    ...
    ex:IsBig {IsBig}.
            ...
            with open("./LLM_only/initial_onto.ttl", "a") as file:
                file.write(addition)
            with open("./LLM_only/updated_onto.ttl", "a") as file:
                file.write(addition)
...
def object_correction(LLM_output):
    O, R, T, IM = fetch_details("./LLM_only/updated_onto.ttl")
    A = O+R+T+IM
    a, f = fetch_actions()
    fl = [k.split("_")[0] for k in f]
    g = rdflib.Graph()
    g.parse("./LLM_only/updated_onto.ttl", format='turtle')
...
def function_correction(LLM_output, n):
    ...
    kg = rdflib.Graph()
    kg.parse("./LLM_only/knowledge_graph.ttl", format='turtle')
    ...
    #             stuff[index]=f'{i[0]}("{i[1]}","{common_recep("updated_onto.ttl")}")\n'
    #             funcs_list[index][-1] = common_recep("updated_onto.ttl")
    ...
    #             print(f'{fun[0]}("{fun[-1]}") #item "{fun[1]}" is not present in ther knowledge base!')

    #     elif len(fun)==3:
    #         if fun[0] not in f:
    #             print(f'{fun[0]}("{fun[1]}", "{fun[-1]}") #action "{fun[0]}" is not present in ther knowledge base!')
    #         if fun[1] not in A and not fun[1].isdigit():
    #             print(f'{fun[0]}("{fun[1]}", "{fun[-1]}") #item "{fun[1]}" is not present in ther knowledge base!')

    #     elif len(fun)==4:
    #         if fun[0] not in f:
    #             print(f'{fun[0]}("{fun[1]}", "{fun[2]}", "{fun[-1]}") #action "{fun[0]}" is not present in ther knowledge base!')
    #         if fun[1] not in A and not fun[1].isdigit():
    #             print(f'{fun[0]}("{fun[1]}", "{fun[2]}", "{fun[-1]}")#item "{fun[1]}" is not present in ther knowledge base!')
    #         if fun[2] not in A and not fun[2].isdigit():
    #             print(f'{fun[0]}("{fun[1]}", "{fun[2]}", "{fun[-1]}")#item "{fun[2]}" is not present in ther knowledge base!')
            ...
            if len(f)>2 and not f[0].split("_")[0]=='put'and not f[0].split("_")[0]=='pour':
                if i==0:
                    if change_initial_onto==0:
                        if len(f)==3:
                            stuff[i]=f'{f[0]}("{f[1]}", "./LLM_only/initial_onto.ttl")\n'
                        elif len(f)==4:
                            stuff[i]=f'{f[0]}("{f[1]}", "{f[2]}", "./LLM_only/initial_onto.ttl")\n'
                    else:
                        if len(f)==3:
                            stuff[i]=f'{f[0]}("{f[1]}", "./LLM_only/updated_onto.ttl")\n'
                        elif len(f)==4:
                            stuff[i]=f'{f[0]}("{f[1]}", "{f[2]}", "./LLM_only/updated_onto.ttl")\n'
                else:
                    if len(f)==3:
                        stuff[i]=f'{f[0]}("{f[1]}", "./LLM_only/updated_onto.ttl")\n'
                    elif len(f)==4:
...
def self_correction(LLM_output, change_initial_onto, n):
    ...
    function_correction(LLM_output, n)
...
def ask_gpt(main_prompt, my_prompt):
    ...
    response = client.chat.completions.create(
        model="gpt-4o",  # Adjust the model as per your need
        messages=[
    ...
    with open("./LLM_only/LLM_orig_output.txt", "w") as file:
...
def back_to_LLM(main_prompt, my_prompt, co, i):
    ...
    if os.path.exists("./LLM_only/outputs/output_with_error.txt"):
        ...
        with open("./LLM_only/outputs/output_with_error.txt", "r") as file:
            lines = file.readlines()

        ll = [l for l in lines if l.strip() != '' and 'Error' in l.split()[0]]

            
    with open (f"./LLM_only/feedback/LLM_feedback_with_error_{i}.txt" , "w") as file:
        file.write(issue)

    remove_indentation(f"./LLM_only/feedback/LLM_feedback_with_error_{i}.txt")
    with open (f"./LLM_only/feedback/LLM_feedback_with_error_{i}.txt" , "a") as file:   
        file.write("\n")
    ...
    LLM_feedback_with_error = triple(f"./LLM_only/feedback/LLM_feedback_with_error_{i}.txt")


    client = OpenAI()

    LLM_orig_output = triple("./LLM_only/LLM_orig_output.txt")
    ...
    if i==0:
        response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": triple(f"./LLM_only/feedback/LLM_feedback_with_error_{i}.txt")}
                ],
                stream = True
            )
        ...
        response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": main_prompt},
                    {"role": "user", "content": my_prompt },
                    {"role": "assistant", "content": triple(f"./LLM_only/feedback/LLM_updated_orig_output_{i}.txt")},
                    {"role": "user", "content": triple(f"./LLM_only/feedback/LLM_feedback_with_error_{i}.txt")}
                ],
                stream = True
            )
    ...
    with open(f"./LLM_only/feedback/LLM_updated_orig_output_{i+1}.txt", "w") as file:
        for chunk in response:
            if chunk.choices[0].delta.content is not None:
                file.write(chunk.choices[0].delta.content)
...
def correct_LLM_output(orig_file, cleaned_file, change_initial_onto, n):
    ...
    remove_serial_numbers(cleaned_file)
    remove_comments(cleaned_file)
    so = self_correction(cleaned_file, change_initial_onto, n)
    if not so:
        print("\nLLM failed to give the action sequence!\n")
...
def text_2_py(file):
    ...
    with open(file,'r') as firstfile, open('./LLM_only/final_run.py','w') as secondfile: 
...
def success(dish):
     ...
     with open("./LLM_only/kitchen.json", "r") as file:
     ...
     if os.path.exists("./LLM_only/feedback/LLM_updated_orig_output_3.txt"):
          of = "./LLM_only/feedback/LLM_updated_orig_output_3.txt"

     else:
          of = "./LLM_only/LLM_orig_output.txt"

     copy_contents(of, "./LLM_only/LLM_orig_output_copy.txt")
     
     remove_empty_lines("./LLM_only/LLM_orig_output_copy.txt")

     with open("./LLM_only/LLM_orig_output_copy.txt", 'r') as file:
     ...
     with open("./LLM_only/for_metric/f.txt", "w") as file_1, open("./LLM_only/for_metric/s.txt", "w") as file_2, open("./LLM_only/for_metric/t.txt", "w") as file_3 :
     ...
     correct_everything("./LLM_only/for_metric/f.txt")
     correct_everything("./LLM_only/for_metric/s.txt")
     correct_everything("./LLM_only/for_metric/t.txt")


     text_2_py("./LLM_only/for_metric/f.txt") 
     remove_indentation('./LLM_only/final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('./LLM_only/final_run.py', "./LLM_only/for_metric/f_output.txt", "./LLM_only/for_metric/f_output_with_error.txt" )


     text_2_py("./LLM_only/for_metric/s.txt") 
     remove_indentation('./LLM_only/final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('./LLM_only/final_run.py', "./LLM_only/for_metric/s_output.txt", "./LLM_only/for_metric/s_output_with_error.txt" )

     text_2_py("./LLM_only/for_metric/t.txt") 
     remove_indentation('./LLM_only/final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('./LLM_only/final_run.py', "./LLM_only/for_metric/t_output.txt", "./LLM_only/for_metric/t_output_with_error.txt" )
     ...
     with open("./LLM_only/for_metric/f_output.txt", "r") as file_1, open("./LLM_only/for_metric/s_output.txt", "r") as file_2, open("./LLM_only/for_metric/t_output.txt", "r") as file_3 :
          
          f_lines = file_1.readlines()
          s_lines = file_2.readlines()
          t_lines = file_3.readlines()

     if not f_lines or os.path.exists("./LLM_only/for_metric/f_output_with_error.txt"):
           ff = 0

     if not s_lines or os.path.exists("./LLM_only/for_metric/s_output_with_error.txt"):
           ss = 0

     if not t_lines or os.path.exists("./LLM_only/for_metric/t_output_with_error.txt"):
           tt = 0
     ...
     sum = ff + ss + tt
     os.remove("./LLM_only/LLM_orig_output_copy.txt")
...
def fetch_actions():
    with open("./LLM_only/functions/actions.py", "r") as file:
        lines = file.readlines()
...
def ingd_used():
    ...
    O, R, T, IM = fetch_details("./LLM_only/initial_onto.ttl")
 
    with open("./LLM_only/final_output.txt", "r") as file:
        stuff = file.readlines()
...
def remove_unknown(number_of_feedback):
    ...
    if os.path.exists("./LLM_only/feedback/LLM_updated_orig_output.txt"):
        of = f"./LLM_only/feedback/LLM_updated_output_{number_of_feedback+1}.txt"

    else:
        of = "./LLM_only/LLM_output.txt"
...
def progress_lines():
    ...
    def prog_lines(output):
        ...
        os.makedirs("./LLM_only/prog_lines", exist_ok=True)
        with open("./LLM_only/prog_lines/progress_lines.txt", "w") as file:
            file.writelines(lines)
    ...
    with open("./LLM_only/final_output.txt") as file:
        lines = file.readlines()

    for i, f in enumerate(lines):
        for obj in o:
            if obj in lines[i] and obj.split("_")[0] in lines[i].replace("_", " ").split():
                obj_used.append(obj)
        for tool in t:
            if tool in lines[i] and tool.split("_")[0] in lines[i].replace("_", " ").split():
                tool_used.append(tool)
        for rec in r:
            if rec in lines[i] and rec.split("_")[0] in lines[i].replace("_", " ").split():
                rec_used.append(rec)
    ...
    prog_lines("./LLM_only/final_output.txt")

    with open("./LLM_only/prog_lines/progress_lines.txt", "r") as file:
        lines = file.readlines()
    ...
    for o in obj_used:
        ...
        for i,l in enumerate(lines):
            ...
            if o in lines[i] and o.split("_")[0] in lines[i].replace("_", " ").split():
    ...
    for t in tool_used:
        ...
        for i,l in enumerate(lines):
            ...
            if t in lines[i] and t.split("_")[0] in lines[i].replace("_", " ").split():
                a.append(lines[i].strip())
    ...
    for r in rec_used:
        ...
        for i,l in enumerate(lines):
            ...
            if r in lines[i] and r.split("_")[0] in lines[i].replace("_", " ").split():
                a.append(lines[i].strip())
        
        all_recs.append(a)


    with open("./LLM_only/prog_lines/obj_prog_lines.txt", "w") as file:
    ...
    with open("./LLM_only/prog_lines/tool_prog_lines.txt", "w") as file:
    ...
    with open("./LLM_only/prog_lines/rec_prog_lines.txt", "w") as file:
...
def gen_final():
    ...
    if os.path.exists("./LLM_only/outputs/output_with_error.txt"):
        ...
        for i in range(1, len(os.listdir("./LLM_only/outputs"))):
            with open(f"./LLM_only/outputs/output_{i}.txt", "r") as file:
                lines = file.readlines()
        ...
        for i in range(1, len(os.listdir("./LLM_only/outputs"))+1):
            with open(f"./LLM_only/outputs/output_{i}.txt", "r") as file:
                lines = file.readlines()
            all_lines = all_lines + lines
    ...
    with open("./LLM_only/final_output.txt", "w") as file:
...
def count_errors():
    ...
    if len(os.listdir("./LLM_only/feedback")) !=0:
        j = len(os.listdir("./LLM_only/feedback"))//3
