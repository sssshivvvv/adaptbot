...
def move(loc, onto_file):
    ...
    if iloc==loc:
        ...
        g.serialize(destination="./LLM_only/updated_onto.ttl", format="turtle")
...
def pick_up_obj(obj, onto_file):
    ...
    if obj in o:
        ...
        raise RuntimeError(f"pick_up_obj({obj}, {onto_file}) #Cant pick {obj}! It is immobile")
    ...
    """
    b = kg.query(big_query)

    for s in b:

        if s.status:
            raise RuntimeError(f"pick_up_obj({obj}, {onto_file}) #{obj} is a big object, cant pick it!")
    ...
    """
    sstatus = kg.query(sliceable_query)
...
def put_down_obj(obj, loc):
    ...
    if obj in R+T+IM:
        raise RuntimeError(f"put_down_obj({obj}, {loc}) #Wrong function choice!")
    ...
    """
    hq = kg.query(bighold_query)
    for h in hq:
        if h.status:
            raise RuntimeError(f"put_down_obj({obj}, {loc}) #couldnt pick {obj}! It is big")
    ...
    if obj not in objs:
        ...
        pick_up_obj(obj, "./LLM_only/updated_onto.ttl")
    ...
    if loc in R:
        ...
        m = move(loc_list[0], "./LLM_only/updated_onto.ttl")  # Agent first moves to the target_mrec location
        ...
        m = move(loc_list[0], "./LLM_only/updated_onto.ttl")  # Agent first moves to the target_object location


    else:
        
        m = move(loc, "./LLM_only/updated_onto.ttl") #move to the target location
...
def pick_up_rec(rec, onto_file):
    ...
    if rec in r:
        ...
        raise RuntimeError(f"pick_up_rec({rec}, {onto_file}) #Cant pick {rec}! It is immobile")
    ...
    for s in b:
        ...
        if s.status:
            raise RuntimeError(f"pick_up_rec({rec}, {onto_file}) #{rec}  is a big receptacle, cant pick it!")
...
def put_down_rec(rec, loc):
    ...
    if rec in O+T:
        raise RuntimeError(f"put_down_rec({rec}, {loc}) #Wrong function choice!")
    
    if rec in IM:
        raise RuntimeError(f"put_down_rec({rec}, {loc}) #couldnt pick {rec}! skipping this action")
    ...
    """
    cantake = kg.query(cantake_query)

    for s in cantake:
        if not s.status:
            raise RuntimeError(f"put_down_rec({rec}, {loc}) #Cant put {rec} to {loc}!")
    ...
    """
    hq = kg.query(hold_query)
    for h in hq:
        if h.status:
            raise RuntimeError(f"put_down_rec({rec}, {loc}) #couldnt pick {rec}! It is big")
    ...
    if rec not in objs:
        pick_up_rec(rec, "./LLM_only/updated_onto.ttl")
...
def pick_up_tool(tool, onto_file):
    ...
    if tool in t:
        ...
        raise RuntimeError(f"pick_up_tool({tool}, {onto_file}) #Cant pick {tool}! It is a immobile receptacle")
...
def put_down_tool(tool, loc):
    ...
    if tool in O+R+IM:
        raise RuntimeError(f"put_down_tool({tool}, {loc}) #Wrong function choice!")
    ...
    if tool not in objs:
        pick_up_tool(tool, "./LLM_only/updated_onto.ttl")
...
def clean(item, onto_file):
    ...
    if item in o:
        ...
        """
        cstatus = kg.query(cleanable_query)

        for s in cstatus:

            if not s.status:
                raise RuntimeError(f"clean({item}, {onto_file}) #{item} doesn't need to be cleaned!")
...
def slice(obj, tool, onto_file):
    ...
    if obj not in O and obj in R+T+IM:
            raise RuntimeError(f"slice({obj}, {tool}, {onto_file}) #{obj} cant be sliced!")
    ...
    """
    objs = kg.query(obj_query)
    obj_list = []
    for o in objs:
        obj_list.append(str(o.obj_name))

    if obj not in obj_list:
       raise RuntimeError(f"slice({obj}, {tool}, {onto_file}) #{obj} is not Sliceable!")
    ...
    for s in status:
        if s.status:
            ...
            """
            kg_status = kg.query(clean_kg_query)

            for s, skg in zip(status, kg_status):

                if not s.status and not skg.cstatus:                
                    clean(obj, onto_file)
                    onto_file = "./LLM_only/updated_onto.ttl"
                    g = rdflib.Graph()
                    g.parse("./LLM_only/updated_onto.ttl", format='turtle')
            ...
            pick_up_tool(tool, "./LLM_only/updated_onto.ttl")
            move(random.sample(slice_boards,1)[0], "./LLM_only/updated_onto.ttl")
            print(f"{obj} sliced!")
...
def boil(obj, onto_file):
    ...
    if obj not in obj_list:
       raise RuntimeError(f"boil({obj}, {onto_file}) #{obj} is not boilable!")
...
def fry(obj, onto_file):
    ...
    if obj not in obj_list:
        ...
        raise RuntimeError(f"fry({obj}, {onto_file}) #{obj} is not Fryable!")
...
def pour_contents(from_rec, to_rec):
    ...
    if not contents:
        raise RuntimeError(f"pour_contents({from_rec}, {to_rec}) #{from_rec} is empty!")
    ...
    if loc_locn not in orplaces and loc_locn not in R:
        put_down_rec(to_rec, common_recep("./LLM_only/updated_onto.ttl"))
        put_down_rec(from_rec, common_recep("./LLM_only/updated_onto.ttl"))
...
def switch(rec, onto_file):
    ...
    if rec not in A:
        ...
        raise RuntimeError(f"switch({rec}, {onto_file}) #{rec} not toggleable!")
...
def mop(rec, onto_file):
    ...
    if rec not in A:
        ...
        raise RuntimeError(f"switch({rec}, {onto_file}) #{rec} cant be mopped!")
...
def iron(obj, onto_file):
    ...
    if obj not in A:
        raise RuntimeError(f"iron({obj}, {onto_file}) #{obj} not present in the environment!")
    
    elif obj in IM+T+R:
        print(f"{obj} cant be ironed! skipping this action")
        return False
    ...
    ironable = []
    for i in m:
        ironable.append(str(i.obj_name))

    if obj not in ironable:
        print(f"cant iron {obj}!")
        return False
    ...
    SELECT ?status
    WHERE {{
        ex:{obj} ex:can_be_ironed true.
        ex:{obj} ex:is_ironed ?status.
    }}
    """
    s = g.query(iron_query)
    for i in s:
        if i.status:
            print(f"{obj} already ironed!")
            return True

    
    pick_up_obj(obj, onto_file)
    put_down_obj(obj, "ironing_board")
    pick_up_rec("iron", "./LLM_only/updated_onto.ttl")
    switch("iron", "./LLM_only/updated_onto.ttl")
    print(f"agent iron the {obj} successfully!")
...
def fold(obj, onto_file):
    ...
    if obj not in A:
        raise RuntimeError(f"fold({obj}, {onto_file}) #{obj} not present in the environment!")
    
    elif obj in IM+T+R:
        print(f"{obj} cant be folded! skipping this action")
        return False
    ...
    WHERE {{
        ex:{obj} ex:can_be_ironed true.
        ex:{obj} ex:folded ?status.
    }}
    """
    s = g.query(iron_query)
    for i in s:
        if i.status:
            print(f"{obj} already folded!")
            return True

    
    pick_up_obj(obj, onto_file)
    put_down_obj(obj, "ironing_board")
    print(f"agent folded the {obj} successfully!")
