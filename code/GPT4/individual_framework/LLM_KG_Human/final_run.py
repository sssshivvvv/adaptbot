import rdflib
from functions.actions import *
pick_up_obj("chicken", "initial_onto.ttl")
put_down_obj("chicken", "countertop")
put_down_obj("onion", "countertop")
put_down_obj("tomato", "countertop")
put_down_obj("bread", "countertop")
put_down_obj("salt", "countertop")
put_down_obj("pepper", "countertop")
put_down_obj("oil", "countertop")
put_down_tool("knife", "countertop")
put_down_rec("cutting_board", "countertop")
put_down_rec("pot", "stove")
put_down_tool("spatula", "countertop")
slice("chicken", "knife", "updated_onto.ttl")
slice("onion", "knife", "updated_onto.ttl")
slice("tomato", "knife", "updated_onto.ttl")
pour_contents("oil", "pot")
fry("chicken", "updated_onto.ttl")
fry("onion", "updated_onto.ttl")
fry("tomato", "updated_onto.ttl")
pour_contents("salt", "pot")
pour_contents("pepper", "pot")
stir_contents("pot", "updated_onto.ttl")
put_down_rec("plate", "countertop")
put_down_obj("bread", "plate")
pour_contents("pot", "cake_pan")
put_down_obj("chicken", "countertop")
put_down_obj("onion", "countertop")
put_down_obj("tomato", "countertop")
put_down_obj("bread", "countertop")
put_down_obj("salt", "countertop")
put_down_obj("pepper", "countertop")
put_down_obj("oil", "countertop")
put_down_tool("knife", "countertop")
put_down_rec("cutting_board", "countertop")
put_down_rec("pot", "stove")
put_down_tool("spatula", "countertop")
slice("chicken", "knife", "updated_onto.ttl")
slice("onion", "knife", "updated_onto.ttl")
slice("tomato", "knife", "updated_onto.ttl")
pour_contents("oil", "pot")
fry("chicken", "updated_onto.ttl")
fry("onion", "updated_onto.ttl")
fry("tomato", "updated_onto.ttl")
pour_contents("salt", "pot")
pour_contents("pepper", "pot")
stir_contents("pot", "updated_onto.ttl")
put_down_rec("plate", "countertop")
put_down_obj("bread", "plate")
pour_contents("pot", "cake_pan")
