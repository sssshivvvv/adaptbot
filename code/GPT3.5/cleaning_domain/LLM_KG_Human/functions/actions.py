...
def move(loc, onto_file):
    ...
    if iloc==loc:
        ...
        g.serialize(destination="./LLM_KG_Human/updated_onto.ttl", format="turtle")
...
def put_down_obj(obj, loc):
    ...
    if obj not in objs:
        ...
        pick_up_obj(obj, "./LLM_KG_Human/updated_onto.ttl")
    ...
    if loc in R:
        ...
        m = move(loc_list[0], "./LLM_KG_Human/updated_onto.ttl")  # Agent first moves to the target_mrec location
        ...
        m = move(loc_list[0], "./LLM_KG_Human/updated_onto.ttl")  # Agent first moves to the target_object location


    else:
        
        m = move(loc, "./LLM_KG_Human/updated_onto.ttl") #move to the target location
...
def put_down_rec(rec, loc):
    ...
    if rec in O+T:
        print("Wrong function choice! skipping this action")
        return False
    ...
    if rec not in objs:
        pick_up_rec(rec, "./LLM_KG_Human/updated_onto.ttl")
...
def put_down_tool(tool, loc):
    ...
    if tool not in objs:
        pick_up_tool(tool, "./LLM_KG_Human/updated_onto.ttl")
        ...
        if s.status:
            ...
            for s in status:
                ...
                if not s.status and s.cstatus:                
                    clean(obj, onto_file)
                    onto_file = "./LLM_KG_Human/updated_onto.ttl"
                    g = rdflib.Graph()
                    g.parse("./LLM_KG_Human/updated_onto.ttl", format='turtle')
            ...
            pick_up_tool(tool, "./LLM_KG_Human/updated_onto.ttl")
            move(random.sample(slice_boards,1)[0], "./LLM_KG_Human/updated_onto.ttl")
            print(f"{obj} sliced!")
        ...
        if b.status:
            ...
            for i in IsL:
                ...
                if i.status:
                    ...
                    switch("stove", "./LLM_KG_Human/updated_onto.ttl")
...
def pour_contents(from_rec, to_rec):
    ...
    if loc_locn not in orplaces and loc_locn not in R:
        put_down_rec(to_rec, common_recep("./LLM_KG_Human/updated_onto.ttl"))
        put_down_rec(from_rec, common_recep("./LLM_KG_Human/updated_onto.ttl"))
...
def iron(obj, onto_file):
    ...
    pick_up_rec("iron", "./LLM_KG_Human/updated_onto.ttl")
    switch("iron", "./LLM_KG_Human/updated_onto.ttl")
