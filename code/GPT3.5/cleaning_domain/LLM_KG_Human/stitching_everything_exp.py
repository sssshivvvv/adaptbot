import rdflib
from LLM_KG_Human.utils.actions import *
...
if os.path.exists("./LLM_KG_Human/outputs"):
    delete_folder_contents("./LLM_KG_Human/outputs")
os.makedirs("./LLM_KG_Human/outputs", exist_ok=True)
...
def LLM_KG_Human(main_prompt, my_prompt, n_fb, iter, maintok, dish_name, dish_ingd):
    ...
    if n_fb == 0: #no_feedback
        ...
        if co:
            ...
            run_py('./LLM_KG_Human/final_run.py', "./LLM_KG_Human/outputs/output_1.txt", "./LLM_KG_Human/outputs/output_with_error.txt" )
        ...
        while need_feedback and i< int(n_fb):
            ...
            if co and RBO=='':
                ...
                remove_indentation('./LLM_KG_Human/final_run.py')
                run_py('./LLM_KG_Human/final_run.py', f"./LLM_KG_Human/outputs.output_{i+2}.txt", "./LLM_KG_Human/outputs/output_with_error.txt" )

                if not os.path.exists("./LLM_KG_Human/outputs/output_with_error.txt"):
                    print("feedback not needed!")
                else:
                    need_feedback = True
        ...
        if need_feedback and i==int(n_fb): #human involvement
            ...
            if RBO!='':
                ...
                remove_indentation('./LLM_KG_Human/final_run.py')
                print("\n")
                run_py('./LLM_KG_Human/final_run.py', f"./LLM_KG_Human/outputs/output_{i+1}.txt", "./LLM_KG_Human/output_with_error.txt" )
    ...
    if os.path.exists("./LLM_KG_Human/outputs/output_with_error.txt"):
        os.remove("./LLM_KG_Human/outputs/output_with_error.txt")  
    ...
    for file_name in os.listdir("./LLM_KG_Human/outputs"):
        if file_name.startswith('output_') and file_name.endswith('.txt'):
            shutil.copy(f"./LLM_KG_Human/outputs/{file_name}", f"./LLM_KG_Human/experiment_results/results_{iter}/Execution_outputs_{iter}")

    shutil.copy("./LLM_KG_Human/final_output.txt", f"./LLM_KG_Human/experiment_results/results_{iter}/Execution_outputs_{iter}")

    if os.path.exists("./LLM_KG_Human/outputs/output_with_error.txt"):
        shutil.copy("./LLM_KG_Human/outputs/output_with_error.txt", f"./LLM_KG_Human/experiment_results/results_{iter}/Execution_outputs_{iter}")
