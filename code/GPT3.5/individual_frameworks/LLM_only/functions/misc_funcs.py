...
ex:{item} rdf:type ex:object;
    ...
    ex:IsBig {IsBig}.
            ...
            with open("initial_onto.ttl", "a") as file:
                file.write(addition)
            with open("updated_onto.ttl", "a") as file:
                file.write(addition)
            ...
            if len(f)>2 and not f[0].split("_")[0]=='put'and not f[0].split("_")[0]=='pour':
                if i==0:
                    if change_initial_onto==0:
                        if len(f)==3:
                            stuff[i]=f'{f[0]}("{f[1]}", "initial_onto.ttl")\n'
                        elif len(f)==4:
                            stuff[i]=f'{f[0]}("{f[1]}", "{f[2]}", "initial_onto.ttl")\n'
                    else:
                        if len(f)==3:
                            stuff[i]=f'{f[0]}("{f[1]}", "updated_onto.ttl")\n'
                        elif len(f)==4:
                            stuff[i]=f'{f[0]}("{f[1]}", "{f[2]}", "updated_onto.ttl")\n'
                else:
                    if len(f)==3:
                        stuff[i]=f'{f[0]}("{f[1]}", "updated_onto.ttl")\n'
                    elif len(f)==4:
...
def back_to_LLM(main_prompt, my_prompt, co, i):
    ...
    if os.path.exists("./outputs/output_with_error.txt"):
            
        with open("./outputs/output_with_error.txt", "r") as file:
            lines = file.readlines()

        ll = [l for l in lines if l.strip() != '' and 'Error' in l.split()[0]]

            
    with open (f"./feedback/LLM_feedback_with_error_{i}.txt" , "w") as file:
        file.write(issue)

    remove_indentation(f"./feedback/LLM_feedback_with_error_{i}.txt")
    with open (f"./feedback/LLM_feedback_with_error_{i}.txt" , "a") as file:   
        file.write("\n")
    ...
    LLM_feedback_with_error = triple(f"./feedback/LLM_feedback_with_error_{i}.txt")
    ...
    LLM_orig_output = triple("LLM_orig_output.txt")
    ...
    if i==0:
        response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": triple(f"./feedback/LLM_feedback_with_error_{i}.txt")}
                ]
            )
        ...
        response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": main_prompt},
                    {"role": "user", "content": my_prompt },
                    {"role": "assistant", "content": triple(f"./feedback/LLM_updated_orig_output_{i}.txt")},
                    {"role": "user", "content": triple(f"./feedback/LLM_feedback_with_error_{i}.txt")}
                ]
            )
    ...
    with open(f"./feedback/LLM_updated_orig_output_{i+1}.txt", "w") as file:
        file.write(str(response.choices[0].message.content))
...
def text_2_py(file):
    ...
    with open(file,'r') as firstfile, open('final_run.py','w') as secondfile: 
...
def success(dish):
     ...
     if os.path.exists("./feedback/LLM_updated_orig_output_3.txt"):
          of = "./feedback/LLM_updated_orig_output_3.txt"

     else:
          of = "LLM_orig_output.txt"

     copy_contents(of, "LLM_orig_output_copy.txt")
     
     remove_empty_lines("LLM_orig_output_copy.txt")

     with open("LLM_orig_output_copy.txt", 'r') as file:
     ...
     with open("./for_metric/f.txt", "w") as file_1, open("./for_metric/s.txt", "w") as file_2, open("./for_metric/t.txt", "w") as file_3 :
     ...
     correct_everything("./for_metric/f.txt")
     correct_everything("./for_metric/s.txt")
     correct_everything("./for_metric/t.txt")


     text_2_py("./for_metric/f.txt") 
     remove_indentation('final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('final_run.py', "./for_metric/f_output.txt", "./for_metric/f_output_with_error.txt" )


     text_2_py("./for_metric/s.txt") 
     remove_indentation('final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('final_run.py', "./for_metric/s_output.txt", "./for_metric/s_output_with_error.txt" )

     text_2_py("./for_metric/t.txt") 
     remove_indentation('final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('final_run.py', "./for_metric/t_output.txt", "./for_metric/t_output_with_error.txt" )
     ...
     with open("./for_metric/f_output.txt", "r") as file_1, open("./for_metric/s_output.txt", "r") as file_2, open("./for_metric/t_output.txt", "r") as file_3 :
          
          f_lines = file_1.readlines()
          s_lines = file_2.readlines()
          t_lines = file_3.readlines()

     if not f_lines or os.path.exists("./for_metric/f_output_with_error.txt"):
           ff = 0

     if not s_lines or os.path.exists("./for_metric/s_output_with_error.txt"):
           ss = 0

     if not t_lines or os.path.exists("./for_metric/t_output_with_error.txt"):
           tt = 0
     ...
     sum = ff + ss + tt
     os.remove("LLM_orig_output_copy.txt")
...
def fetch_actions():
    with open("./functions/actions.py", "r") as file:
        lines = file.readlines()
...
def ingd_used():
    ...
    O, R, T, IM = fetch_details("initial_onto.ttl")
...
def remove_unknown(number_of_feedback):
    ...
    if os.path.exists("./feedback/LLM_updated_orig_output.txt"):
        of = f"./feedback/LLM_updated_output_{number_of_feedback+1}.txt"
...
def progress_lines():
    ...
    def prog_lines(output):
        ...
        os.makedirs("prog_lines", exist_ok=True)
        with open("./prog_lines/progress_lines.txt", "w") as file:
            file.writelines(lines)
    ...
    prog_lines("final_output.txt")

    with open("./prog_lines/progress_lines.txt", "r") as file:
        lines = file.readlines()
    ...
    with open("./prog_lines/obj_prog_lines.txt", "w") as file:
    ...
    with open("./prog_lines/tool_prog_lines.txt", "w") as file:
    ...
    with open("./prog_lines/rec_prog_lines.txt", "w") as file:
...
def gen_final():
    ...
    if os.path.exists("./outputs/output_with_error.txt"):
        ...
        for i in range(1, len(os.listdir("outputs"))):
            with open(f"./outputs/output_{i}.txt", "r") as file:
                lines = file.readlines()
        ...
        for i in range(1, len(os.listdir("outputs"))+1):
            with open(f"./outputs/output_{i}.txt", "r") as file:
                lines = file.readlines()
            all_lines = all_lines + lines
...
def count_errors():
    i = 0
    j = 0
    if os.path.exists("output_with_error.txt"):
        i = 1

    if len(os.listdir("feedback")) !=0:
        j = len(os.listdir("feedback"))//3
