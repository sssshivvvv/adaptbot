...
def update_ontology(item):
        ...
        kind = input(f"What kind of item is {item}? Choose from: 'object', 'mreceptacle', 'imreceptacle', 'tool'\n")
        ...
        if kind =='object':
            ...
            mop = input("Is object mopping material? Please reply in true and false\n")
            fab = input("Is object fabric? Please reply in true and false\n")
            iron = input("Can object be ironed? Please reply in true and false\n")
            foldable = input("Is object foldable? Please reply in true and false\n")
...
ex:{item} rdf:type ex:object;
    ...
    ex:mopping_material {mop};
    ex:is_fabric {fab};
    ex:is_ironed false;
    ex:can_be_ironed {iron};
    ex:foldable {foldable};
    ex:folded false.
                """
            
            with open("./LLM_KG_Human/initial_onto.ttl", "a") as file:
                file.write(addition)
            with open("./LLM_KG_Human/updated_onto.ttl", "a") as file:
                file.write(addition)
...
def object_correction(LLM_output):
    O, R, T, IM = fetch_details("./LLM_KG_Human/updated_onto.ttl")
    A = O+R+T+IM
    a, f = fetch_actions()
    fl = [k.split("_")[0] for k in f]
    g = rdflib.Graph()
    g.parse("./LLM_KG_Human/updated_onto.ttl", format='turtle')
        ...
        if i and len(i)>1:
            ...
            if 'put' in i[0]: #function_asscociation_correction
                ...
                if i[1] in O and i[-1] not in R+IM+O:
                    ...
                    stuff[index]=f'{i[0]}("{i[1]}","{common_recep("./LLM_KG_Human/updated_onto.ttl")}")\n'
                    funcs_list[index][-1] = common_recep("./LLM_KG_Human/updated_onto.ttl")
        ...
        if fun:
            ...
            if len(fun)==2:
                ...
                if fun[1] not in A and not fun[1].isdigit():
                    print(f'{fun[0]}("{fun[-1]}") #item "{fun[1]}" not found! Please use the items provided by the user')

            elif len(fun)==3:
                if fun[0] not in f:
                    print(f'{fun[0]}("{fun[1]}", "{fun[-1]}") #action "{fun[0]}" is not present in ther knowledge base!')
                if fun[1] not in A and not fun[1].isdigit():
                    print(f'{fun[0]}("{fun[1]}", "{fun[-1]}") #item "{fun[1]}" not found! Please use the items provided by the user')

            elif len(fun)==4:
                if fun[0] not in f:
                    print(f'{fun[0]}("{fun[1]}", "{fun[2]}", "{fun[0]}") #action "{fun[0]}" is not present in ther knowledge base!')
                if fun[1] not in A and not fun[1].isdigit():
                    print(f'{fun[0]}("{fun[1]}", "{fun[2]}", "{fun[1]}") #item "{fun[1]}" not found! Please use the items provided by the user')
                if fun[2] not in A and not fun[2].isdigit():
                    print(f'{fun[0]}("{fun[1]}", "{fun[2]}", "{fun[2]}") #item "{fun[2]}" not found! Please use the items provided by the user')
...
def knowledge_expansion(LLM_output):
        ...
        for i, f in enumerate(func_list):
            ...
            if len(f)==3:
                if f[1]==wrong_item:
                    ...
                    func_list[i][2] = correct_item
                    stuff[i] = f'{f[0]}("{f[1]}", "{correct_item}")\n'
                ...
                if f[1]==wrong_item:
                    ...
                    func_list[i][2] = correct_item
                    stuff[i] = f'{f[0]}("{f[1]}", "{correct_item}", "{f[-1]}")\n'              
    ...
    def rectify_function(stuff, funcs_list):
        ...
        nonlocal Human_involvement
        nonlocal corrections_made
        nonlocal additions_made
        ...
        for index, fun in enumerate(funcs_list):
            ...
            if fun and len(fun)>1:   
                ...
                if fun[0] not in f:
                    repeat = True
                    while repeat:
                        ask = input(f'{stuff[index].strip()} #The action "{fun[0]}" does not exist in my knowledge base!Is it present in the system? Please answer in "yes" or "no". If I am mistaking it for another function, please enter "correct_function".\n')
                        Human_involvement+=1
    ...
    stuff, funcs_list = get_funcs(LLM_output)  
    return Human_involvement, additions_made, corrections_made
            ...
            if len(f)>2 and not f[0].split("_")[0]=='put'and not f[0].split("_")[0]=='pour':
                if i==0:
                    if change_initial_onto==0:
                        if len(f)==3:
                            stuff[i]=f'{f[0]}("{f[1]}", "./LLM_KG_Human/initial_onto.ttl")\n'
                        elif len(f)==4:
                            stuff[i]=f'{f[0]}("{f[1]}", "{f[2]}", "./LLM_KG_Human/initial_onto.ttl")\n'
                    else:
                        if len(f)==3:
                            stuff[i]=f'{f[0]}("{f[1]}", "./LLM_KG_Human/updated_onto.ttl")\n'
                        elif len(f)==4:
                            stuff[i]=f'{f[0]}("{f[1]}", "{f[2]}", "./LLM_KG_Human/updated_onto.ttl")\n'
                else:
                    if len(f)==3:
                        stuff[i]=f'{f[0]}("{f[1]}", "./LLM_KG_Human/updated_onto.ttl")\n'
                    elif len(f)==4:
...
def back_to_LLM(main_prompt, my_prompt, RBO, co, i):
    ...
    issue = """ 
    There were some issue(s) in the action sequence you gave. The error is reflected below. Give me the cot and task sequence again. Make sure to strictly USE All THE items GIVEN BY THE USER IN THE PROMPT.
    """
    ...
    if os.path.exists("./LLM_KG_Human/outputs/output_with_error.txt"):
            
        with open("./LLM_KG_Human/outputs/output_with_error.txt", "r") as file:
            lines = file.readlines()
...
def text_2_py(file):
    ...
    with open(file,'r') as firstfile, open('./LLM_KG_Human/final_run.py','w') as secondfile: 
...
def success(dish):
     ...
     with open("./LLM_KG_Human/kitchen.json", "r") as file:
     ...
     if os.path.exists("./LLM_KG_Human/feedback/LLM_updated_orig_output_3.txt"):
          of = "./LLM_KG_Human/feedback/LLM_updated_orig_output_3.txt"

     else:
          of = "./LLM_KG_Human/LLM_orig_output.txt"

     copy_contents(of, "./LLM_KG_Human/LLM_orig_output_copy.txt")
     
     remove_empty_lines("./LLM_KG_Human/LLM_orig_output_copy.txt")

     with open("./LLM_KG_Human/LLM_orig_output_copy.txt", 'r') as file:
     ...
     with open("./LLM_KG_Human/for_metric/f.txt", "w") as file_1, open("./LLM_KG_Human/for_metric/s.txt", "w") as file_2, open("./LLM_KG_Human/for_metric/t.txt", "w") as file_3 :
     ...
     correct_everything("./LLM_KG_Human/for_metric/f.txt")
     correct_everything("./LLM_KG_Human/for_metric/s.txt")
     correct_everything("./LLM_KG_Human/for_metric/t.txt")


     text_2_py("./LLM_KG_Human/for_metric/f.txt") 
     remove_indentation('./LLM_KG_Human/final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('./LLM_KG_Human/final_run.py', "./LLM_KG_Human/for_metric/f_output.txt", "./LLM_KG_Human/for_metric/f_output_with_error.txt" )


     text_2_py("./LLM_KG_Human/for_metric/s.txt") 
     remove_indentation('./LLM_KG_Human/final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('./LLM_KG_Human/final_run.py', "./LLM_KG_Human/for_metric/s_output.txt", "./LLM_KG_Human/for_metric/s_output_with_error.txt" )

     text_2_py("./LLM_KG_Human/for_metric/t.txt") 
     remove_indentation('./LLM_KG_Human/final_run.py')
     with io.StringIO() as buf, contextlib.redirect_stdout(buf):
          run_py('./LLM_KG_Human/final_run.py', "./LLM_KG_Human/for_metric/t_output.txt", "./LLM_KG_Human/for_metric/t_output_with_error.txt" )
     ...
     with open("./LLM_KG_Human/for_metric/f_output.txt", "r") as file_1, open("./LLM_KG_Human/for_metric/s_output.txt", "r") as file_2, open("./LLM_KG_Human/for_metric/t_output.txt", "r") as file_3 :
          
          f_lines = file_1.readlines()
          s_lines = file_2.readlines()
          t_lines = file_3.readlines()

     if not f_lines or os.path.exists("./LLM_KG_Human/for_metric/f_output_with_error.txt"):
           ff = 0

     if not s_lines or os.path.exists("./LLM_KG_Human/for_metric/s_output_with_error.txt"):
           ss = 0

     if not t_lines or os.path.exists("./LLM_KG_Human/for_metric/t_output_with_error.txt"):
           tt = 0
     ...
     sum = ff + ss + tt
     os.remove("./LLM_KG_Human/LLM_orig_output_copy.txt")
...
# items will help us check if the minimum required items have been used or not.
def ingd_used():

    gen_final()

    O, R, T, IM = fetch_details("./LLM_KG_Human/initial_onto.ttl")
    A = O+R+T+IM
    with open("./LLM_KG_Human/final_output.txt", "r") as file:
        stuff = file.readlines()
...
def remove_unknown(number_of_feedback):
    ...
    if os.path.exists("./LLM_KG_Human/feedback/LLM_updated_orig_output.txt"):
        of = f"./LLM_KG_Human/feedback/LLM_updated_output_{number_of_feedback+1}.txt"

    else:
        of = "./LLM_KG_Human/LLM_output.txt"
...
def progress_lines():
    ...
    def prog_lines(output):
        ...
        os.makedirs("./LLM_KG_Human/prog_lines", exist_ok=True)
        with open("./LLM_KG_Human/prog_lines/progress_lines.txt", "w") as file:
            file.writelines(lines)
    ...
    with open("./LLM_KG_Human/final_output.txt") as file:
        lines = file.readlines()
    ...
    prog_lines("./LLM_KG_Human/final_output.txt")

    with open("./LLM_KG_Human/prog_lines/progress_lines.txt", "r") as file:
        lines = file.readlines()
    ...
    with open("./LLM_KG_Human/prog_lines/obj_prog_lines.txt", "w") as file:
    ...
    with open("./LLM_KG_Human/prog_lines/tool_prog_lines.txt", "w") as file:
    ...
    with open("./LLM_KG_Human/prog_lines/rec_prog_lines.txt", "w") as file:
...
def gen_final():
    ...
    # Get all filenames
    file_names = [f for f in os.listdir("LLM_KG_Human/outputs") if f.startswith('output_') and f.endswith('.txt')]
    ...
    if os.path.exists("./LLM_KG_Human/outputs/output_with_error.txt"):
        ...
        for f in file_names:
            with open(f"./LLM_KG_Human/outputs/{f}", "r") as file:
                lines = file.readlines()
    ...
    with open("./LLM_KG_Human/final_output.txt", "w") as file:
